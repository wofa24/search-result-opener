...
function addCard(index, screenshot, title, url, tabId, total, favIconUrl = '') {
  ...
  // 检查是否已存在（增量更新逻辑修复）
  let card = grid.querySelector(`.card[data-tab-id="${tabId}"]`);
  if (card) return;
  ...
  card.innerHTML = `
    <div class="thumbnail-container" id="thumb-${index}">
      ...
      <div class="number-badge">${currentCount + 1}</div>
    </div>
    <div class="card-info">
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'initNavigation') {
    // 接收初始同步信息，更新 Header
    const data = request.data;
    document.getElementById('partNumber').textContent = data.partNumber || '-';
    document.getElementById('supplier').textContent = data.supplier || '-';
    document.getElementById('searchEngine').textContent = data.searchEngine || 'Bing';
    document.getElementById('totalCount').textContent = data.total || 0;
    document.getElementById('resultCount').textContent = data.total || 0;
    sendResponse({ success: true });
  } else if (request.action === 'addScreenshot') {
    // 添加或更新卡片基本信息

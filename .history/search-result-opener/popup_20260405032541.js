...
async function waitForTabAndAutomate(tabId, partNumber, supplier, count, createGroup, openSidebar, shouldCloseOnFinish = false) {
  return new Promise(async (resolve) => {
    ...
    while (extractedLinks.length < count && pageAttempts < MAX_PAGES) {
      ...
      showStatus(`正在抓取页面 ${pageAttempts + 1} [已获: ${extractedLinks.length}/${count}]...`, 'info');
      ...
      try {
        const results = await chrome.scripting.executeScript({
          ...
          func: async (limit, currentTotal) => {
            const pageLinks = [];
            const seen = new Set();
            const selectors = ['#b_results .b_algo', '.b_algo', '#rso .g', '.g', '.rc'];
            
            // 内部滚动以加载懒加载项
            let scrolls = 0;
            while ((currentTotal + pageLinks.length) < limit && scrolls < 3) {
                let items = [];
                ...
                for (const item of items) {
                    const a = item.querySelector('h2 a') || item.querySelector('a[href]');
                    ...
                    if (url.startsWith('http') && !seen.has(url)) {
                        ...
                        seen.add(url);
                        pageLinks.push({ url, title: tEl ? tEl.textContent.trim() : url, favIconUrl: `https://www.google.com/s2/favicons?sz=64&domain=${new URL(url).hostname}`});
                    }

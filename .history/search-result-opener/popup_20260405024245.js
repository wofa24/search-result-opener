...
async function handleConfirm() {
  ...
  try {
    ...
    // 智能复用：如果当前页已经是搜索页，直接在当前页更新，不再新开 tab
    if (isSearchPage(tab.url)) {
      showStatus('正在当前页发起搜索...', 'info');
      await chrome.tabs.update(tab.id, { url: searchUrl });
      await waitForTabAndAutomate(tab.id, partNumber, supplier, count, createGroup, openSidebar, false);
    } else {
      showStatus('正在跳转新标签搜索...', 'info');
      const newTab = await chrome.tabs.create({ url: searchUrl, active: true });
      // 如果是从非搜索页（如空白页）新开的，完成后可以考虑自动关闭它
      await waitForTabAndAutomate(newTab.id, partNumber, supplier, count, createGroup, openSidebar, true);
    }
